...
// CreateUserWithID provides a mock function with given fields: id, dto
func (_m *UserRepository) CreateUserWithID(id uint, dto *entities.CreateUserData) error {
	ret := _m.Called(id, dto)

	if len(ret) == 0 {
		panic("no return value specified for CreateUserWithID")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(uint, *entities.CreateUserData) error); ok {
		r0 = rf(id, dto)
	} else {
		r0 = ret.Error(0)
	}
...
func (_m *UserRepository) DeleteUsers() error {
	ret := _m.Called()
